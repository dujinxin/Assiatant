...
public class JXSelectView: UIView {
    ...
    public var selectSecondRow : Int = 0
    ...
    func showView() {
        self.show(inView: self.bgWindow, animate: true)
    }
        ...
        if animate {
            UIView.animate(withDuration: animateDuration, delay: 0.0, options: .curveEaseIn, animations: {
                ...
                if self.style == .list {
                    ...
                    if self.selectRow >= 0 {
                        self.pickView.selectRow(self.selectRow, inComponent: 0, animated: false)
                    }
                    if self.selectSecondRow >= 0 {
                        self.pickView.selectRow(self.selectSecondRow, inComponent: 1, animated: false)
                    }
                }
            })
        }
    }
    public func dismissView(animate: Bool = true) {
...
extension JXSelectView : UIPickerViewDelegate, UIPickerViewDataSource{
    ...
    public func pickerView(_ pickerView: UIPickerView, viewForRow row: Int, forComponent component: Int, reusing view: UIView?) -> UIView {
        pickerView.subviews.forEach { (view) in
            if view.frame.size.height < 2{
                view.backgroundColor = UIColor.rgbColor(rgbValue: 0xeeeeee)
            }
        }
        var string = ""
        if let dataSource = self.dataSource {
